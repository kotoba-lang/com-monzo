(ns monzo.main
  "Kotodama WASM entrypoint for the Monzo clean-room actor (L4) — Clojure port.
  ...
  facts (`monzo.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Account"    :plural "accounts"    :id-prefix "monzo_acc"
    :fields [:id :description :type :created] :required [:description]
    :coerce {}                       :refs {}}
   {:entity "Balance"    :plural "balances"    :id-prefix "monzo_bal"
    :fields [:balance :totalBalance :currency :spendToday] :required [:balance :totalBalance]
    :coerce {:balance :int :totalBalance :int :spendToday :int} :refs {}}
   {:entity "Transaction" :plural "transactions" :id-prefix "monzo_tra"
    :fields [:id :amount :currency :description :category :declineReason :isLoad :merchant :created :settled] :required [:amount]
    :coerce {:amount :int :isLoad :bool} :refs {}}
   {:entity "Pot"       :plural "pots"       :id-prefix "monzo_pot"
    :fields [:id :name :balance :currency :deleted :created :updated] :required [:name]
    :coerce {:balance :int :deleted :bool} :refs {}}
   {:entity "Attachment" :plural "attachments" :id-prefix "monzo_att"
    :fields [:id :userId :externalId :fileUrl :fileType :created] :required [:fileUrl]
    :coerce {}                       :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"monzo.<Entity>/<field>\" v ...}. The datomic
...
(defn handle-create [store entity data]
  (let [{:keys [fields required coerce id-prefix]} (spec-for entity)]
    (or (some-> (reject-unknown data fields) (vector 400))
        ...
        (let [base (if (contains? (set required) :id) {:id (get data :id)} {:id (new-id id-prefix)})
              rec (reduce (fn [m f] (if (= f :id) m (assoc m f (coerce-field (get coerce f) (get data f))))) base fields)
...
(defn healthz [] [{:status "ok" :actor "monzo-compat" :tier tier :entities entities} 200])
